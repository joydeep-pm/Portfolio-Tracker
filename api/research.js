...
const ROUTE_PATHS = {
  "earnings-chat": "/api/v1/research/earnings/chat",
  "earnings-sync": "/api/v1/research/earnings/sync",
};
...
async function forwardRequest(pathname, body, traceId) {
  ...
  try {
    ...
    return {
      status: response.status,
      payload: {
        error: "research-non-json-response",
        message: text || "Research worker returned non-JSON payload",
      },
    };
  } finally {
...
module.exports = async function handler(req, res) {
  const trace = initTrace(req, res, "research-proxy-api");
  ...
  if (!targetPath) {
    return json(
      ...
      withMeta(
        {
          error: "not-found",
          message: "Supported routes: earnings-chat, earnings-sync",
        },
        trace.traceId,
      ),
    );
  ...
  try {
    ...
    traceLog(trace, "info", "research.proxy.success", {
      route,
    ...
    const message = timedOut ? "Research worker request timed out" : error.message || "Research worker unavailable";
    traceLog(trace, "error", "research.proxy.failed", { route, message });
    return json(
      res,
      statusCode,
      withMeta(
        {
          error: timedOut ? "research-timeout" : "research-proxy-failed",
          message,
        },
        trace.traceId,
      ),
